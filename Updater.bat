...
ECHO.
ECHO Файлы удаленные в папке "...KPD\Source documents pdf":>>"%root_directory%\KPD\# Source documents docx, doc, xls, xlsx\Files updated on %date% by notification No. %notification%\Deletion.txt"
ECHO.>>"%root_directory%\KPD\# Source documents docx, doc, xls, xlsx\Files updated on %date% by notification No. %notification%\Deletion.txt"
...
ECHO !var_incr_del_pdf!^) *.pdf файл для документа %%G был найден и удален>>"%root_directory%\KPD\# Source documents docx, doc, xls, xlsx\Files updated on %date% by notification No. %notification%\Deletion.txt"
SET /a var_incr_del_pdf+=1
