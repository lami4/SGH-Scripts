...
Function Replace-FilesInArchive ($currentDirectoryName)
{
    ...
    (New-Object -COM Shell.Application).NameSpace("$desktopPath\$folderWithProcessedDocuments\waste").MoveHere("$desktopPath\$folderWithProcessedDocuments\$currentDirectoryName.zip\word\media")
    ...
    for ($i = 0; $i -lt $imageProperties[0].Length; $i++) 
        ...
        if ($existenceInImageStorage -eq $true)
            ...
            if ($currentExtension -eq $imageInStorage[0].Extension)
                ...
                Copy-Item -Path $imageInStorage[0] "$desktopPath\$folderWithProcessedDocuments\media"
                $fileToBeRenamed = $imageInStorage[0].Name
                Rename-Item -Path "$desktopPath\$folderWithProcessedDocuments\media\$fileToBeRenamed" -NewName "$currentFullName"
                } else {
                    if ($currentExtension -eq ".wdp") {
                    #convert to bmp first as WDP converter can work only with BMP files
                    magick convert $imageInStorage[0] "$desktopPath\$folderWithProcessedDocuments\converted-to-bmp\$currentName.bmp"
                    #then BMP files is converted to WDP
                    Start-Process -FilePath 'C:\WDP Converter\JXREncApp\x64\JXREncApp.exe' -ArgumentList "-i ""$desktopPath\$folderWithProcessedDocuments\converted-to-bmp\$currentName.bmp"" -o ""$desktopPath\$folderWithProcessedDocuments\media\$currentName.wdp"" -c 0"
                    } else {
                    ...
                    magick convert $imageInStorage[0] "$desktopPath\$folderWithProcessedDocuments\media\$currentFullName"
                    }
                    }
            } else {
            if ($currentExtension -eq ".wdp") {
