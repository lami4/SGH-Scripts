...
Function Generate-XmlList ($List, [ValidateSet("Departments", "Employees", "Projects")]$ListType)
{
...
Function Manage-CustomLists ($PathToLost, [ValidateSet("Departments", "Employees", "Projects")]$ListType)
{
    ...
    if ($ListType -eq "Projects") {$ManageCustomListsForm.Text = "Редактировать список проектов"}
    ...
    if ($ListType -eq "Projects") {$GetPropertyLabelBlacklistListBox.Text = "Список проектов компании:"}
    ...
    if ($ListType -eq "Projects") {if (Test-Path "$PSScriptRoot\Проекты.xml") {Populate-List -List $GetPropertyListBoxBlackList -PathToXml "$PSScriptRoot\Проекты.xml"}}
    ...
    if ($ListType -eq "Projects") {$GetPropertyInputboxAddItem.Text = "Укажите название проекта..."}
    $GetPropertyInputboxAddItem.ForeColor = "Gray"
    $GetPropertyInputboxAddItem.Add_GotFocus({
        if ($GetPropertyInputboxAddItem.Text -eq "Укажите название отдела..." -or $GetPropertyInputboxAddItem.Text -eq "Укажите ФИО..." -or $GetPropertyInputboxAddItem.Text -eq "Укажите название проекта...") {
    ...
    $GetPropertyButtonApplyItem.Add_Click({
        if ($GetPropertyInputboxEditItem.Text -eq $GetPropertyListBoxBlackList.SelectedItem) {
            ...
            if ($GetPropertyListBoxBlackList.Items.Contains($GetPropertyInputboxEditItem.Text)) {
                ...
                if ($ListType -eq "Projects") {Show-MessageBox -Title "Запись уже существует в списке" -Type OK -Message "Список уже содержит указанный проект ($($GetPropertyInputboxEditItem.Text))."}
            } else {
    ...
    if ($ListType -eq "Projects") {$GetPropertyLabelButtonDelete.Text = "Удалить проект из списка"}
...
Function UniquenessCheckForm ()
{
    ...
    $UniquenessCheckFormRegisterLabel.Width = 500
