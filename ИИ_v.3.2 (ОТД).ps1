        ...
        if ($_.Cells.Count -eq 7) {
            if (((($_.Cells.Item(4).Range.Text).Trim([char]0x0007)).Trim(' ') -replace [char]13, '') -ne '') {
                if ($script:CollectedReferences[0] -cnotcontains [string]((($_.Cells.Item(4).Range.Text).Trim([char]0x0007)).Trim(' ')  -replace [char]13, '').Trim([char]0x0009)) {
                    $script:CollectedReferences[0] += [string](((($_.Cells.Item(4).Range.Text).Trim([char]0x0007)).Trim(' ')  -replace [char]13, ' ').Trim([char]0x0009).Trim(' ') -replace '\s+', ' ')
                    $script:CollectedReferences[1] += [string](((($_.Cells.Item(5).Range.Text).Trim([char]0x0007)).Trim(' ')  -replace [char]13, ' ').Trim([char]0x0009).Trim(' ') -replace '\s+', ' ')
                    $script:CollectedReferences[2] += [string](((($_.Cells.Item(1).Range.Text).Trim([char]0x0007)).Trim(' ')  -replace [char]13, ' ').Trim([char]0x0009).Trim(' ') -replace '\s+', ' ')
                }         
            }
        }
    }
    $script:CollectedReferences[0] += [string](((($Specification.Sections.Item(1).Footers.Item(2).Range.Tables.Item(1).Cell(1, 6).Range.Text)).Trim(' ')  -replace [char]13, ' ').Trim([char]0x0007).Trim([char]0x0009).Trim(' ') -replace '\s+', ' ')
    $script:CollectedReferences[1] += [string](((($Specification.Sections.Item(1).Footers.Item(2).Range.Tables.Item(1).Cell(4, 5).Range.Text)).Trim(' ')  -replace [char]13, ' ').Trim([char]0x0007).Trim([char]0x0009).Trim(' ')  -replace '\s+', ' ')
        ...
        if ($_.SubItems[2].Text -eq "Документ") {
            ...
            if ($script:CollectedReferences[0].Contains("$($_.Text)")) {
                if ($script:CollectedReferences[1][$script:CollectedReferences[0].IndexOf("$($_.Text)")] -ne "") {
                    ...
                    if (Test-Path -Path "$script:SelectedFolderWithFilesBeingPublished\$($_.Text).docx") {
...
Function ClientReleaseForm ()
{
    ...
    if ($script:SelectedClientFolder -eq $null -or (($script:SelectedWordFile -eq $null -and $ClientReleaseFormMultipleSpecs.Checked -eq $false) -or ($ClientReleaseFormMultipleSpecs.Checked -eq $true -and $script:SelectedSpecsFolder -eq $null))) {
        ...
        if ((Show-MessageBox -Message "Перед началом операции убедитесь в том, что у вас нет открытых Word документов.`r`nВо время работы скрипт закроет все Word документы, не сохраняя их, что может привести к потере данных!`r`nПродолжить?" -Title "Подтвердите действие" -Type YesNo) -eq "Yes") {
            ...
            if ($ClientReleaseFormMultipleSpecs.Checked -eq $true) {$CollectFromMultipleSpecsFlag = $true} else {$CollectFromMultipleSpecsFlag = $false}
            if (Test-Path -Path "$PSScriptRoot\Релиз для клиента.html") {
...
Function Create-ClientVersion ($PathToSpecification, $PathToClientFolder, $CollectFromMultipleSpecsFlag, $DeleteWordFlag, $DeleteExcelFlag)
{
    Kill -Name WINWORD -ErrorAction SilentlyContinue
    ...
    $script:counter = 1
    $CatchedArchives = @()
    if ($CollectFromMultipleSpecsFlag -eq $true) {
        ###
        Write-Host "Собираю ссылки на архивы исходных кодов..."
        #Создать экземпляр приложения MS Word
        $WordApp = New-Object -ComObject Word.Application
        #Сделать вызванное приложение невидемым
        $WordApp.Visible = $false
        Get-ChildItem -Path "$script:SelectedSpecsFolder\*.docx" | % {
            if ($_.BaseName -match 'SPC' -or $_.BaseName -match 'LPD') {
                $WordDocumentSpecification = $WordApp.Documents.Open($_.FullName)
                $WordDocumentSpecification.Tables.Item(1).Rows | % {
                    if ($_.Cells.Count -eq 7 -and ((($_.Cells.Item(5).Range.Text).Trim([char]0x0007)).Trim(' ') -replace [char]13, '') -match "архив исходных кодов") {
                        if ($CatchedArchives -cnotcontains [string]((($_.Cells.Item(4).Range.Text).Trim([char]0x0007)).Trim(' ') -replace [char]13, '').Trim([char]0x0009)) {
                            $CatchedArchives += [string](((($_.Cells.Item(4).Range.Text).Trim([char]0x0007)).Trim(' ')  -replace [char]13, ' ').Trim([char]0x0009).Trim(' '))
                        }
                    }
                }
        ...
        Get-ChildItem -Path "$script:SelectedSpecsFolder\*.doc" | % {
            if ($_.BaseName -match 'SPC' -or $_.BaseName -match 'LPD') {
                $WordDocumentSpecification = $WordApp.Documents.Open($_.FullName)
                $WordDocumentSpecification.Tables.Item(1).Rows | % {
                    if ($_.Cells.Count -eq 7 -and ((($_.Cells.Item(5).Range.Text).Trim([char]0x0007)).Trim(' ') -replace [char]13, '') -match "архив исходных кодов") {
    ...
    Kill -Name WINWORD -ErrorAction SilentlyContinue
    if ($CollectFromMultipleSpecsFlag -eq $false) {Write-Host "Удаляю архивы, указанные в спецификации, из указанной папки..."}
    if ($CollectFromMultipleSpecsFlag -eq $true) {Write-Host "Удаляю архивы, указанные в спецификацих, из указанной папки..."}
    Start-Sleep -Seconds 2

    Add-Content "$PSScriptRoot\Релиз для клиента.html" "<!DOCTYPE html>
<html lang=""ru"">
<head>
<meta charset=""utf-8"">
<title>Отчет об удалении архивов с исходным кодом</title>
<style type=""text/css"">
div {
font-family: Verdana, Arial, Helvetica, sans-serif;
}
...
<body>
<div>
<h4>Отчет о действиях, выполненных во время удаления архивов, содержащих исходный код:</h4>
<div>
</div>
<table style=""width:100%"">
    <tr>
...
Start-Sleep -Seconds 1
    $ErrorCounter = 1
    $CatchedArchives | % {
        if (Test-Path -Path "$($PathToClientFolder)\$_") {
            Remove-Item -Path "$($PathToClientFolder)\$_"
            Add-Content "$PSScriptRoot\Релиз для клиента.html" "    <tr>
            <td class=""Item_Number"">$($ErrorCounter)</td>
            <td class=""File"">$($_)</td>
            <td class=""Action"">Удаление</td>
            <td class=""Details""><font color=""green""><b>Удален.</b></font> Архив был найден в указанной папке и удален.</td>
        </tr>" -Encoding UTF8
            $ErrorCounter += 1 
        } else {
            Add-Content "$PSScriptRoot\Релиз для клиента.html" "    <tr>
            ...
            <td class=""File"">$($_)</td>
            <td class=""Action"">Удаление</td>
            <td class=""Details""><font color=""red""><b>Ошибка.</b></font> Одна или несколько спецификаций ссылаются на архив, которого не существует в указанной папке. В спецификации либо неверно указано название архива, либо название содержит запрещенные спецсимволы, либо спецификация ссылается на несуществующий файл.</td>
        </tr>" -Encoding UTF8
            $ErrorCounter += 1  
        }
    }
Add-Content "$PSScriptRoot\Релиз для клиента.html" "</table>
</div>
</body>
</html>" -Encoding UTF8
