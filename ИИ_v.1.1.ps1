...
Function Copy-FileToFolder ($FileFullName, $Destination, [ValidateSet("Backup", "Default")]$Type)
{
...
Function Check-Conditions ($BackupFlag)
{
    ...
    $ListViewRemove.Items | % {
        if ($_.SubItems[2].Text -eq "Документ") {
            if ((Test-Path -Path "$script:MakeChangesPathToCurrentVersion\$($_.Text).*") -eq $false) {$ErrorsToBePublished += "Документ <i>$($_.Text)</i> находится в списке <b>Аннулировать</b>, но отсутствует в папке с публикуемыми файлами."; $ErrorDetectedFlag = $true}
        }
        if ($_.SubItems[2].Text -eq "Программа") {
            if ((Test-Path -Path "$script:MakeChangesPathToCurrentVersion\$($_.Text)") -eq $false) {$ErrorsToBePublished += "Программа <i>$($_.Text)</i> находится в списке <b>Аннулировать</b>, но отсутствует в папке с публикуемыми файлами."; $ErrorDetectedFlag = $true}
        }  
    }
    #ПРОВЕРКИ
